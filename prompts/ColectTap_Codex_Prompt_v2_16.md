...
```javascript
${mostrarOutro ? campo('codigo_projeto_outro', 'Código de Projeto (especificar)', 'text', {placeholder: 'Ex: NR-12, EN 13445…'}) : ''}
${campo('ano_edicao_codigo_projeto', 'Ano de Edição do Código', 'number', {placeholder: 'Ex: 2019'})}
```
...
const codigoProjetoDisplay = (d.codigo_projeto === 'Outro'
  ...
  : (d.codigo_projeto || '—')) +
  (d.ano_edicao_codigo_projeto ? ' (' + d.ano_edicao_codigo_projeto + ')' : '');
...
## BLOCO 13 — GAS: CAMPO ano_edicao_codigo_projeto

**Nenhuma alteração necessária no GAS.**

O `ColectTap_GAS_v1_16.js` já suporta o campo `ano_edicao_codigo_projeto` nativamente:
- Já está no array de campos aceitos em `salvarLevantamentoNR13` (linha 703)
- Já está no objeto de retorno de `obterEquipamentoPorOS` (linha 398)

**Portanto, o GAS v1.17 é idêntico ao v1.16 — apenas bump de versão.**
...
□ ano_edicao_codigo_projeto está no GAS payload, no objeto de retorno e na lista de migrarEstrutura
